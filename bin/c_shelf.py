...
def _editions_for_family(ctx, family):
    rows = [r for r in ctx.rel_to_row.values() if r["family"] == family]
...
def do_find(ctx, queries, pool=200):
    ...
    for fam, score in ranked:
        ...
        card = ctx.shelf.execute("SELECT catalog FROM cards WHERE rel=?", (best_rel,)).fetchone()
        catalog = card[0] if card else ""
        editions, n_copies, kind = _editions_for_family(ctx, fam)
        ek = best_row["edition_key"]
        primary_rel = best_rel
        if not best_row["is_primary"]:
            for r in ctx.rel_to_row.values():
                if r["edition_key"] == ek and r["is_primary"]:
                    primary_rel = r["rel"]
                    break
        families_out.append({
...
def do_have(ctx, words_str, fy=None):
    ...
    # is a JSON blob echoing a real file's path (e.g. a "profile" record for
    # "Sources/.../Pakistan Economic Survey ....pdf"), and those get long,
    # noisy `family` text that out-scores the real 4-word title on raw bm25
    # term frequency if `family` is included in this first tier.
    rows = []
...
# series
# --------------------------------------------------------------------- #
def do_series(ctx, row_words, family_words, fy_from=None, fy_to=None):
    ...
    out, matched = [], 0
    for r in members:
        ins = do_inside(ctx, r["rel"], row_words, k=1)
        if ins["hits"]:
            matched += 1
            h = ins["hits"][0]
            ...
            out.append({"fy": r["fy_primary"], "rel": r["rel"], "page_index": None, "line": None})
