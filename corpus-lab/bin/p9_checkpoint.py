...
RUN_ID = "P9-2026-09-15"
BRANCH = "phase9-production-hardening"
CKPT = L.STATE / "phase9_checkpoint.json"
BEAT = L.STATE / "phase9_heartbeat.log"
...
def _progress_tail(out):
    ...
    if p.exists():
        for line in p.read_text(encoding="utf-8", errors="replace").splitlines():
            ...
            if j.get("phase") == "P9":
                lines.append(j)
    out.append(("", "last %d P9 progress lines:" % min(5, len(lines))))
    for j in lines[-5:]:
        note = str(j.get("note", ""))
        if len(note) > 160:
...
def cmd_resume():
    ...
    print("=" * 72)
    print("PHASE 9 RESUME   run_id=%s  last_beat=%s" % (d["run_id"], d["last_beat"]))
    print("=" * 72)
