# p10_variance_s2 — Sonnet repeat 2 of 3
...
The second of three identical Sonnet batteries. It changes nothing and tests nothing new.
Its only job is to be a second point, so that the first one stops being mistaken for a
measurement.
...
- `_private/results/03_runs/P10S2_live/` — the recording, made once
- `corpus-lab/state/c_live_battery_v2__P10S2_live.json`
- `corpus-lab/state/c_live_battery_v3__P10S2_live.json`
- `corpus-lab/state/c_live_battery_v3__P10S2_live__keyv2.json`
- `corpus-lab/state/phase9_probes__P10S2.json` — the three isolation probes
...
`cited_right_page_equiv` **11 of 17** under v3, **10 of 17** under v2. Strict 2.
`cited_unopened` 6, `mentioned_unopened` 10, answer fragments 10, loop-only opens 0.
Absence 3 of 3. `value_correct` 3 of 5 keyed; `vector_full` 6 of 9 against key v2.
Median wall 81.2s, p90 289.9s. Driver cost **$10.0502**.

Against P10S1's 13 under v3 and 14 under v2, with nothing whatsoever changed between the two
runs. That gap of 2 (v3) and 4 (v2) is the first direct evidence in this project that its
headline moves on its own.

## Adopted?

Not a result to adopt — a data point. What it feeds is the constraint adopted in
[`../p10_variance/README.md`](../p10_variance/README.md).

## Run record

`run_record.json` in this directory — id `p10_P10S2`. The phase-level record is
