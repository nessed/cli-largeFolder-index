...
def _build_table_card_from_lines(caption, heading, body_lines, page_index, kind_hint=None,
    ...
    # 2026-09-13 tuning loop (idea 5 debugging surfaced this): the first
    # non-blank, non-Indicator/Unit/Source line in body_lines is frequently
    # the DATA VALUES row itself (this corpus's layout often prints it
    # directly under the "Indicator ..." header, before the descriptive row
    # label appears later near the Source line) -- picking it unconditionally
    # meant `row_labels` held things like "69.4 78.0 87.0 94.4 112.9 R" for a
    # large fraction of cards, not the actual label text. Skip lines that are
    # mostly digits before accepting one as the row label.
    row_label = None
    for line in body_lines:
        s = line.strip()
        if not s or re.match(r"^(Indicator|Unit\s*:|Source\s*:)", s, re.I):
            continue
        non_space = s.replace(" ", "")
        digit_chars = sum(1 for ch in non_space if ch.isdigit())
        if non_space and digit_chars / len(non_space) > 0.3:
            continue  # looks like a data row, not a label
        row_label = s[:160]
        break
